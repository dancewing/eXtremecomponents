...
public abstract class AbstractLimitFactory implements LimitFactory {
    ...
    protected boolean isAjaxPosted;
    ...
    boolean getAjaxPosted() {
        String ajaxTableId = context.getParameter(TableConstants.AJAX_TABLE_ID);
        if (StringUtils.isBlank(ajaxTableId)) {
            return false;
        }
        if (ajaxTableId.equals(tableId)) {
            return true;
        }
        return false;
    }
